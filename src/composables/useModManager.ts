...
import { Config, Level, SteamCmdConfig } from '../proto/config'
...
export async function updateGameHome(newGameHome: string) {
  try {
    config.value.gameHome = newGameHome
    await save_config()
...
export async function updateSteamCmdHome(newSteamCmdHome: string) {
  try {
    config.value.steamcmdHome = newSteamCmdHome
    await save_config()
...
export async function updateSteamCmdConfig(newSteamCmdConfig: SteamCmdConfig) {
  try {
    config.value.steamcmdConfig = newSteamCmdConfig
    await save_config()
