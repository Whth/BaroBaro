...
import type { Config, SteamCmdConfig } from "../proto/config";
...
export async function saveProfile(profile: Profile) {
	try {
		...
		const existingProfile = mod_lists.value.find(
			(list: ModList) => list.profileName === profile.name,
		);
		if (existingProfile) {
...
export async function updateConfig(newConfig: Config) {
	try {
		...
		const steamcmdHome =
			newConfig.steamcmdHome || config.value.steamcmdHome || "";
...
export function getModById(modId: string): BarotraumaMod | undefined {
	return installed_mod.value.find(
		(mod: BarotraumaMod) => mod.steamWorkshopId === modId,
	);
}

export function getModListByName(profileName: string): ModList | undefined {
	return mod_lists.value.find(
		(list: ModList) => list.profileName === profileName,
	);
}

export function isModEnabled(
	modId: string,
	currentModList: ModList | null,
): boolean {
